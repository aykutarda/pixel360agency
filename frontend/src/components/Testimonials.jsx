...
const Testimonials = () => {
  ...
  const loadData = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/site/sections/testimonials`);
          ...
          {data.items?.map((testimonial, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-accent/20 border border-accent/30 flex items-center justify-center font-pixel text-accent text-sm">
                  {testimonial.avatar}
                </div>
                <div>
